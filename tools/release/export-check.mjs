...
import { getSkillRoot } from "../../scripts/lib/paths.mjs";
...
function main() {
  ...
  const result = {
    ...
    recommendation: !rootExists
      ? "export root is missing; run `bash tools/release/export-standalone.sh` first"
      : !existsSync(runtimeDir) && !existsSync(auditDir)
        ? "release_tree_clean"
        : "run `bash tools/release/clean-local-state.sh --yes` before publishing the standalone repository",
  };
